...
{{-- ================= PRODUITS ================= --}}
@if(($payment->sale && $payment->sale->items->count()) || ($payment->purchase && $payment->purchase->items->count()))
<div class="section">
...
<td class="amount">{{ number_format($item->unit_price,2) }} {{ company()?->currency }}</td>
<td class="amount">{{ number_format($item->total,2) }} {{ company()?->currency }}</td>
</tr>
@endforeach
@endif
