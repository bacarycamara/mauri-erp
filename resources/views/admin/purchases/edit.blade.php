...
<div x-data="purchaseEdit()"
    ...
    <div class="flex justify-between items-start flex-wrap gap-4">
        <div class="flex items-start gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Modifier Achat</h1>
                <p class="text-sm text-gray-500 mt-1">
                    Référence :
                    <span class="font-semibold text-gray-700">{{ $purchase->reference }}</span>
                </p>
            </div>
        </div>
    ...
    <form action="{{ route('admin.purchases.update', $purchase) }}"
        ...
        <div class="bg-white rounded-3xl shadow-sm border p-8">
            <div class="grid md:grid-cols-3 gap-6">
                ...
                <div>
                    <label for="notes"
                           class="block text-sm font-medium text-gray-600 mb-2">Notes</label>
                    <input type="text"
                           id="notes"
                           name="notes"
        ...
        {{-- ================= LIGNES PRODUITS ================= --}}
        <div class="bg-white rounded-3xl shadow-sm border p-8">
            <h2 class="text-lg font-semibold text-gray-800 mb-6">Lignes Produits</h2>
                    ...
                    <tr class="border-t hover:bg-gray-50 transition">
                        ...
                        <td class="px-4 py-3 text-right">
                            <button type="button"
                                    ...
                                    class="text-red-400 hover:text-red-600 p-1 rounded-lg hover:bg-red-50 transition">
        ...
        <div class="flex justify-between items-center flex-wrap gap-4">
            ...
            <form action="{{ route('admin.purchases.destroy', $purchase) }}"
                  method="POST"
                  onsubmit="return confirm('Supprimer définitivement cet achat ?')">
                @csrf
                @method('DELETE')
                <button type="submit"
            ...
            <div class="flex gap-4">
                ...
                <button type="submit"
                        ...
                        class="px-8 py-2.5 bg-green-600 hover:bg-green-700 text-white
                               rounded-2xl shadow transition text-sm
...
function purchaseEdit() {
    return {
        // ✅ Données existantes injectées depuis Blade
        items: @json($purchase->items->map(fn($i) => [
        ...
        addItem() {
            this.items.push({
                ...
                vat_rate: 0, discount_rate: 0, total: 0
            });
        },
        ...
        calculate(index) {
            ...
            i.total = parseFloat((subtotal + vatAmt - discountAmt).toFixed(2));
            this.calculateAll();
        },
